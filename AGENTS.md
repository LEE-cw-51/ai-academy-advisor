...
첫 공개 MVP는 하남 미사 고정의 **무료 AI 보조 학원 탐색 도구**다. 사용자가 현재 다니는 학원 또는 원하는 조건과 고민을 입력하면, AI는 탐색 조건·상담 질문·추가 확인 항목으로 구조화한다. 화면은 출처·확인일이 있는 학원 기본 정보·지도·공개 연락처와 후보 근거를 보여 주고, 사용자가 직접 전화·웹사이트·길찾기를 하게 한다. 미확인 항목은 후보 카드에 나열하지 않고 상담 질문과 상세 정보의 '확인할 점'으로 안내한다 (2026-09-08, `docs/decision-log.md`). 핵심 자산은 UI가 아니라 **출처와 확인일을 갖춘 정확한 학원 사실 데이터베이스**이고, 지금은 유지보수 가능한 MVP가 우선이다.
...
배포된 `/`의 주 CTA는 `/app`(하남 미사 고정, `상황 입력 → 확인해 볼 후보·상담 질문 → 직접 확인`)으로 연결된다. `/app`에서 지도와 검색은 보조 기능이며, 공개 UI는 확정 추천·순위·품질 판정을 약속하지 않는다 (2026-09-13, `docs/decision-log.md`). 보조 퍼널이던 `/check`(1분 학원 점검)·`/checklists`(상담 전 질문)는 퇴역해 `/app`으로 리다이렉트된다 (2026-09-14, `docs/decisions/2026-09-14-retire-check-and-checklists.md`). **자유 대화형 채팅과 SSE 스트리밍(`POST /chat`)은 아직 미구현이며 첫 MVP의 기본 범위도 아니다.**
