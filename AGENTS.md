...
배포된 `/`의 주 CTA는 `/app`(하남 미사 고정, `상황 입력 → 확인해 볼 후보·상담 질문 → 직접 확인`)으로 연결되고, `/check`·`/checklists`는 점검·체크리스트 보조 퍼널이다. `/app`에서 지도와 검색은 보조 기능이며, 공개 UI는 확정 추천·순위·품질 판정을 약속하지 않는다 (2026-09-13, `docs/decision-log.md`). **자유 대화형 채팅과 SSE 스트리밍(`POST /chat`)은 아직 미구현이며 첫 MVP의 기본 범위도 아니다.**
