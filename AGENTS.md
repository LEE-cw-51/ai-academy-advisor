...
배포된 `/`는 출시 전 소개·대기자 페이지다. 제품 뼈대는 **등록 전 · 다니는 중 · 옮기기 전**
세 시점이다. 공개 퍼널 3페이지(`/`·`/check`·`/checklists`)는 같은 히어로 카피와 스티키
헤더·하단 고정 카카오 바를 공유한다. `/`의 본문은 상황 분기(알아보는 중 → `/checklists`,
다니는 중 → `/check`)다. `/check`는 로그인·저장 없는 3문항 정적 도구(인트로 → 문항 →
결과+카카오)다. `/checklists`는 채널 웰컴메시지용 웹 체크리스트 허브다(페이월 없음).
카카오톡 출시 알림은 소개 페이지 하단 고정 바다. ‘하남 미사’ 고정의 **안내형 추천 화면**(학년·학교·
