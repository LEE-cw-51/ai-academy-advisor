 ...
 *  이 컴포넌트가 생기기 전에는 메인(`/`)만 모달을 거쳤고 옛 `/checklists`·`/check` 결과·푸터는
 *  바로 외부로 나갔다 — 같은 약속을 같은 방식으로 하기 위해 한 곳으로 모았다.
 *  모달 상태를 각 호출부가 아니라 여기서 들고 있어 페이지마다 배선을 반복하지 않는다. */
export function KakaoChannelCta({ children, className }: KakaoChannelCtaProps) {
  ...
  return (
    <>
      ...
      <KakaoChannelModal open={open} onClose={closeModal} />
    </>
  );
}
