...
interface SiteChromeProps {
  children: ReactNode;
  /** 법적 푸터. 기본 끔 — `/privacy`만 켠다. */
  footer?: boolean;
  /** 하단 고정 카카오 바. 기본 끔 — `/privacy`만 켠다. */
  kakaoBar?: boolean;
}

/** `/`·`/privacy`가 공유하는 크롬.
 *  푸터·카카오 고정 바는 기본 끔. 홈(`/`)은 히어로만 두고, `/privacy`만 둘 다 켠다
 *  (2026-09-25). 바를 켠 페이지만 `fixed` 바에 가려지지 않게 하단 패딩을 둔다.
...
export function SiteChrome({
  ...
  const bottomPad = kakaoBar
    ? "pb-[calc(8rem+env(safe-area-inset-bottom))]"
    : undefined;

  return (
    <div
      className={["flex min-h-dvh flex-col bg-canvas", bottomPad]
        .filter(Boolean)
        .join(" ")}
    >
      <LandingHeader />
