...
최신 사업 계획의 첫 2개월은 기능 확장보다 검증을 우선한다. 1개월차에는 후보 선정 근거와 상담 질문이 학부모에게 이해되는지 확인하고, 2개월차에는 학부모 5~10명 인터뷰와 비식별 수요 리포트 시안을 통한 원장 반응 검증을 진행한다. 이 검증 계획은 [프로젝트 개요](docs/project.md)와 [로드맵](docs/roadmap.md)에 기록되어 있다.

현재 제공 기능과 후속 검토 기능을 구분한다. 현재는 상황 입력, 상담 질문 구조화, 출처·확인일이 있는 후보 정보, 지도·공개 연락처, 사용자의 직접 확인 행동이 범위다. 상담 기록·비교·저장·선택형 로그인, 학원 대상 유료 리포트, 상담 예약·리드 연결, 인접 지역 확장은 검증 게이트를 통과한 뒤에만 검토한다. 이 저장소 문서는 제품이 학원 품질이나 아이의 상태를 판정하거나 확정 추천한다고 설명하지 않는다.

학원 데이터 수량은 출처와 기준일이 있는 현황으로 다룬다. 특히 `410곳`이라는 표현은 데이터 갱신에 따라 달라질 수 있으므로 기준일·집계 범위가 확인된 경우에만 사용한다. 최신 사업·검증 기준의 원문은 프로젝트 내부의 비공개 제출 자료를 정본으로 복사하지 않고, 공개 저장소에는 그 핵심 원칙과 실행 계획만 기록한다.
