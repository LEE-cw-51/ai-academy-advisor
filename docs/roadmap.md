...
- **P3 기반** 안내형 추천 화면(학년·학교·과목·학습 스타일 입력 폼) + 지도 마커 + 랜딩/대기자 ✅
  — 현재 `/`의 주 CTA는 `/app`(하남 미사 고정, `상황 입력 → 확인해 볼 후보·상담 질문 → 직접 확인`)으로 연결되고, `/check`·`/checklists`는 점검·체크리스트 보조 퍼널이다. 공개 UI는 확정 추천·순위·품질 판정을 약속하지 않는다 (2026-09-13, `docs/decision-log.md`). 랜딩은 `POST /waitlist`를 호출하지 않는다.
