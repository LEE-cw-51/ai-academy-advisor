...
- **P3** 안내형 추천 화면(학년·학교·과목·학습 스타일 입력 폼) + 지도 마커 + 랜딩/대기자 ✅
  — 랜딩(`/`)은 출시 전 대기자 페이지. 약속은 등록 전·다니는 중·옮기기 전 세 시점.
  주 CTA는 1분 학원 점검 → `/check`(재원), 카카오 출시 알림은 보조·하단. `/check`는 점검·체크리스트 카카오 전환.
  `/checklists`는 웹 허브. 추천 UI는 `/app`. 랜딩은 `POST /waitlist`를 호출하지 않는다.
