...
- **서브**: “경기도 공공데이터 기준 미사 지역 등록 학원·교습소 410곳을 모았습니다. 여기에 학년·과목·학습 스타일을 더해 맞는 곳을 추려드리는 서비스를 준비하고 있습니다.”
  - 숫자는 `data/academies/*.json` 실측값이다. **“모았습니다”까지만 쓴다** — 과목·수업형태 등 속성 필드는 아직 전부 `null`이라 “정리 완료”·“조건별 비교”는 거짓이 된다.
- **CTA**: “카카오톡으로 무료 출시 알림 받기” — “무료 · 개인정보 입력 없음 · 언제든 차단 가능”
  - 보조 문구 3항목은 카카오 모달이 약속하는 사실과 1:1로 대응한다. 한쪽만 고치지 않는다.
...
- **모바일 스티키 CTA**: 히어로 CTA를 지나치면 하단에 등장. 상단 고정 CTA는 두지 않는다.
