...
- **주 CTA**: “카카오톡으로 무료 출시 알림 받기” — `WAITLIST_CTA_LABEL` / `CTA_REASSURANCE`
- **출시 후 기능**: “출시하면 이렇게 쓰시게 됩니다” — 맞춤 학원 추천 · 학원 정보 비교 · 상담 연결
- **서비스 화면 예시**: 순위·예시 배지·`OO/△△` 표기 유지 + “왜 추천했나요?” 한 줄. 실제 동 이름·정원 등 구체 속성은 넣지 않는다.
- **대기자**: “출시하면 가장 먼저 알려드릴게요.” / “카카오톡 채널을 추가해 두시면 출시하는 날 알려드릴게요.”
- **모바일 스티키 CTA**: 히어로 CTA **직후** 센티널이 화면 위로 나가면 하단에 등장. 출시 알림 모달 연결.
- **카카오 모달**: “출시 알림을 받으시겠어요?” / “카카오톡 채널 추가하고 알림 받기”. 본문에 체크리스트 3종 혜택 한 줄.
- **푸터**: 출시 전. 점검·체크리스트 웹·출시 알림. 문의 이메일 · 개인정보처리방침 · 카카오톡 채널.
- **`/check`**: 페이지 h1 + 평가 거부 서포트(`ink-muted`) → `1분 학원 점검` CTA + hint. 3문항 Card → 결과(상담 질문은 구분선+라벨) → 카카오 + 홈. 학원 좋음/나쁨 판정 없음.
