...
출시 전 소개·대기자 페이지. 현재형 추천 카피·`/app` CTA·미리보기 목업은 쓰지 않는다.

- **상태**: “정식 출시 준비 중 · 하남 미사” — 히어로 h1 **아래** 배지 한 곳 (헤더에서 반복하지 않는다)
- **히어로**: “하남 미사 학원 410곳, 우리 아이에게 맞는 곳부터”
  - 숫자는 `landingFacts.ts`의 `MISA_ACADEMY_COUNT`이며 `data/academies/*.json` 실측값이다. **“모았습니다”까지만 쓴다** — 과목·수업형태 등 속성 필드는 아직 전부 `null`이라 “정리 완료”·“조건별 비교”는 거짓이 된다.
- **히어로 서포트**: h1 아래 한 줄(“맘카페·블로그를 뒤져도…”) → CTA
- **히어로 CTA 아래 약속** (독립 Pain 섹션·카드 없음): “학년·과목·아이 성향에 맞는 곳을, 근거와 함께 추려드릴 준비를 하고 있어요.”
- **각주**: “410곳 = …” — `text-xs ink-muted`
- **주 CTA**: “1분 학원 점검 시작하기” → `/check` — `HOME_CHECK_CTA_LABEL` / `HOME_CHECK_REASSURANCE`
- **보조 CTA**: “카카오톡으로 무료 출시 알림 받기” — `WAITLIST_CTA_LABEL` (모달)
- **시점 3칸**: “학원을 고를 때, 다닐 때, 옮길 때” — `등록 전` / `다니는 중` / `옮기기 전` (`LIFECYCLE_*` in `landingFacts.ts`). 추천·비교·상담 연결을 같은 칸에 두지 않는다. 등록 전만 출시 후 준비, 나머지 둘은 현재 점검·체크리스트.
- **서비스 화면 예시**: 순위·예시 배지·`OO/△△` 표기 유지 + “왜 추천했나요?” 한 줄. 실제 동 이름·정원 등 구체 속성은 넣지 않는다.
- **대기자**: “출시하면 가장 먼저 알려드릴게요.” / “카카오톡 채널을 추가해 두시면 출시하는 날 알려드릴게요.”
- **모바일 스티키 CTA**: 히어로 CTA **직후** 센티널이 화면 위로 나가면 하단에 등장. 주 행동은 `/check` (`HOME_CHECK_CTA_LABEL` / `STICKY_CHECK_REASSURANCE`). 대기자 모달이 열려 있으면 숨긴다.
- **카카오 모달**: “출시 알림을 받으시겠어요?” / “카카오톡 채널 추가하고 알림 받기”. 본문에 체크리스트 3종 혜택 한 줄.
- **푸터**: 출시 전. 점검·체크리스트 웹·출시 알림. 문의 이메일 · 개인정보처리방침 · 카카오톡 채널.
- **`/check`**: 로고 + 배지 + 두 줄 제목 + 평가 거부 서포트(`ink-muted`) → `1분 학원 점검` CTA + hint. 문항부터는 진행 바+Card. 결과(상담 질문은 구분선+라벨) → 카카오 + 홈. 학원 좋음/나쁨 판정 없음.
- **`/checklists`**: 등록 전·다니는 중·옮기기 전 3섹션. 상단 앵커는 secondary 버튼형. 항목은 `divide-y` 리듬 + brand 번호. PDF 없음.
