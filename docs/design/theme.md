...
- **상태**: “정식 출시 준비 중 · 하남 미사” — 히어로 배지 **한 곳에만** 둔다 (헤더에서 반복하지 않는다)
...
- **히어로 CTA 아래 카드** (독립 Pain 섹션은 두지 않는다): 문제(“맘카페·블로그를 뒤져도…”) / 약속(“학년·과목·아이 성향에 맞는 곳을, 근거와 함께 추려드릴 준비를 하고 있어요.”)
- **각주**: “410곳 = 경기도 공공데이터 기준 미사 지역 등록 학원·교습소”
- **CTA**: “카카오톡으로 무료 출시 알림 받기” — “무료 · 이름·연락처 입력 없음 · 언제든 차단 가능”
  - 보조 문구 3항목은 카카오 모달이 약속하는 사실과 1:1로 대응한다. 문자열은 `landingFacts.ts`의 `CTA_REASSURANCE` 한곳. 모달만 고치지 않는다.
...
- **모바일 스티키 CTA**: 히어로 CTA **직후** 센티널이 화면 위로 나가면 하단에 등장. 상단 고정 CTA는 두지 않는다. 대기자 모달이 열려 있으면 바를 내린다.
...
- **푸터**: 출시 전·알림 신청만 가능. 문의 이메일 · 개인정보처리방침 · 카카오톡 채널. 사업자 정보는 출시 시점에 표기.
  - **유보·고지 문구는 여기 `Disclaimer` 한 곳으로 통합한다** (중개·수강료 대리수납 없음 포함). 본문 섹션마다 반복하지 않는다.
  - 예외: 서비스 화면 예시의 “실제 학원 정보가 아닌 예시”는 그 자리에 있어야 오해를 막으므로 유지한다.
