...
## 2026-08-14 — GitHub를 다중 AI 협업의 단일 허브로 확정 (AGENTS.md 도입)

- **문제**: ChatGPT(전략)·Claude(아키텍처/리뷰)·Cursor(구현)·Manus(조사/실행)를 함께 쓰는데,
  각 AI의 운영 규칙이 **각 도구의 설정 화면에만** 존재했다. 서로의 대화를 볼 수 없고 세션이
  끝나면 맥락이 사라지므로, 채팅창에만 남은 규칙과 결정은 다른 AI에게 존재하지 않는 것과 같다.
  저장소에는 에이전트가 읽을 진입점 파일이 하나도 없었다(`AGENTS.md`/`CLAUDE.md`/`.github/` 부재).
- **결정**: 저장소 루트 **`AGENTS.md`를 모든 에이전트의 단일 공통 계약**으로 삼는다.
  제품 범위, 역할, 계층 규칙, 데이터 정본 원칙, API 계약, 검증 명령, 인계 형식을 여기 모은다.
  도구별 파일(`CLAUDE.md`, `.cursor/rules/project-context.mdc`)은 **얇은 포인터 + 역할 특이사항**만
  두고, 규칙 변경은 도구 설정이 아니라 `AGENTS.md` PR로만 한다.
- **왜 `AGENTS.md`인가**: Cursor·Codex 등 다수 에이전트 도구가 공통으로 읽는 관례 파일이고
  Claude Code도 읽는다. 도구별 규칙 파일 N개를 두면 반드시 서로 어긋나므로 **정본 1개 + 포인터 N개**
  구조를 택했다. 규칙 중복을 발견하면 포인터 쪽을 지운다.
- **인계를 문서가 아니라 템플릿으로 강제**: `.github/pull_request_template.md`가 목적·현재 상태·
  발견/근거·결정·관련 파일·검증 결과·다음 작업·위험을 채우게 하고, 계약 위반 체크리스트
  (하드 엔드포인트 불변, `score` 상대값, `scoring.py` 순수성, 세션 경계, 추측 데이터 금지)를 포함한다.
  Issue 템플릿은 역할별 4종(Research / Task / Decision / Data)으로 나눠 어느 AI가 받을 작업인지
  라벨로 드러낸다.
- **문서 증식 방지**: 새 문서는 `AGENTS.md`와 `docs/ai-team.md` 둘뿐이고, 나머지는 기존 문서 갱신으로
  처리했다. `docs/project.md`가 "OpenAI 연동·RAG·Vector DB를 하지 않는다"고 적힌 채 방치되어
  Phase 4 구현 현실과 반대였으므로 함께 정정했다. 대신 **자유 대화형 채팅·SSE(`POST /chat`)는
  미구현**임을 명시했다 — 로드맵 Phase 5 문구가 이미 구현된 것처럼 읽힐 여지가 있었고,
  실제 `backend/app/api/`에 chat 라우터가 없다.
- **하지 않기로 한 것**: 에이전트별 규칙 파일을 각자 두는 것, GitHub Actions로 규칙을 자동 검사하는 것
  (MVP 단계에서 유지비만 늘고 검증 대상이 아직 유동적이다), 조사 결과 전용 디렉터리 신설
  (Issue + decision-log로 충분하다).
