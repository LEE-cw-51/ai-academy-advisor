...
## 2026-08-27 — 학원 탐색을 ‘상담 질문·관리 구조 투명성’으로 확장하고, 대기행렬 모형은 설명형 보조 신호로 한정

- **계기**: 학원콕의 기존 목표는 학부모가 학원과 아이의 학습 상태를 둘러싼 블랙박스를 줄이고 더 나은 질문과 판단을 하도록 돕는 것이다. Founder가 제공한 *Determining Optimal Class Size in a Private Academy Using Queuing Theory* 과제 논문은 학생 질문을 대기행렬로 해석해, 반 인원만이 아니라 질문 수요·실제 질의응답 시간·동시 대응 인력이 질문 대기와 관리 경험에 영향을 준다는 점을 제시했다. 이 관점은 기존의 진도·오답·보강·질문 대응·소통 문제 정의를 구체적인 상담 질문으로 바꿀 수 있다.
- **제품 방향**: 학원콕의 진입점인 학원 탐색·후보 비교는 유지한다. 다만 서비스의 중심 가치는 단순 추천이 아니라, 학부모가 **상담 전 필요한 질문을 준비하고, 상담 중 학원별 답변을 비교하며, 등록 후 안내받은 관리가 실제로 이행되는지 점검**하도록 돕는 데 둔다. 반의 실제 인원, 동시 질문 대응 인력, 질문이 몰리는 시간의 운영, 오답 피드백, 클리닉·보강 조건은 이 흐름의 핵심 질문 축으로 채택한다.
- **대기행렬 모형의 지위**: M/M/1·M/M/c는 ‘질문 대응 여유’를 이해하기 위한 **내부 보조 모형**으로만 사용한다. 논문 사례의 가동률 임계값과 적정 정원 결과를 모든 학원·과목에 적용하는 공통 기준으로 쓰지 않는다. 계산 또는 설명형 신호는 반·과목·학년군·시간대별 실제 인원, 질문 수요, 질문 가능 시간, 동시 대응 인력, 관측일·출처가 갖춰진 경우에만 후속 실험한다.
...
- **데이터 경계**: `data/academies/*.json`은 출처·확인일이 있는 학원 사실의 유일한 git 정본으로 유지한다. 학원 전체의 `teacher_count`나 `classroom_count`만으로 반별 질문 대응 인력·학생당 강사 수·교육 품질을 추론하지 않는다. 공개 출처의 반 단위 운영값은 향후 별도 운영 프로필로, 등록 후 학부모의 약속 이행 확인은 개인 점검 또는 사용자 데이터로 분리한다. 원시 서술·개인정보는 git 정본에 남기지 않는다.
- **검증 순서**: Stage 1에서 우선 5~10명의 실제 학부모를 대상으로 `질문 대응·오답 관리` 상담 질문의 유용성을 검증한다. 수치 설명이 혼란을 키우거나 운영값을 일관되게 수집할 수 없으면 계산 카드 도입을 보류한다. 그 다음에만 공개 출처·관측일을 갖춘 운영값의 소규모 수집, 등록 후 체감과의 보정, 설명형 신호 공개 여부를 순서대로 검토한다.
- **API·코드 영향**: 이번 결정은 문서·제품 원칙·검증 우선순위의 변경이다. 현재 `GET /academies`, `POST /recommendations`, `POST /recommendations/ai`, 학원 JSON 스키마, 추천 로직, 공개 UI, 로그인·저장 정책은 변경하지 않는다. 따라서 `docs/api.md`, 데이터 마이그레이션, 코드 변경은 이번 범위에 포함하지 않는다.
