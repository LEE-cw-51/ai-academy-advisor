...
### gg — 경기데이터드림/공공데이터포털 "경기도_학원 및 교습소 현황"

강점: 나이스에 없는 **전화번호·좌표(위경도)·교습과정명**(과목 힌트로 활용,
`--course-keyword`로 필터링 가능).

**필드명 미확정.** 두 포털의 상세 페이지가 봇 차단으로 이 프로젝트 개발 중
확인되지 못했다. 검색으로 파악한 필드 개념(시설명·전화번호·주소·좌표·
교습과정명)을 바탕으로 `gg_row_to_record()`가 여러 후보 키를 순서대로
시도하는 best-effort 매핑으로 구현되어 있다. 실제 응답을 확보하면 그 함수의
후보 키 목록만 확인·보정하면 된다 (등록번호는 이 데이터셋에 없는 것으로
보여 자연키는 이름+주소만 사용).

| 개념 | 정본 키 | 비고 |
|---|---|---|
| 시설명 | `name` | |
| 전화번호 | `phone` | neis에는 없는 필드 |
| 도로명주소(우선)/지번주소 | `address` | |
| 위도/경도 | `latitude`/`longitude` | neis에는 없는 필드 |
| 교습과정명 | (필터 전용) | `subjects`에 자동 반영하지 않음 — 과목 매핑 오류 방지, `--course-keyword`로만 사용 |
